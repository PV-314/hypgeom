\\ \r C:\MyStuff\TeXFiles\mypapers\Hypergeometric\My-Yuan-Zhang-Extension\pari\any-b\d-bAll-nAPP-search.gp
...
do_a_b_d_t_u_work(a,b,d,nrmA,t,u,dbg=0)={
	...
	\\ from equation (4.16) (just before (AS-3a)
	yM1Step1UB=0.19*(b*b*abs(nrmA))^(13.0/11)/d^(12.0/11);
	\\ from equation (4.18) (just before (AS-4a)
	yM1Step2UB=abs(nrmA)^(7.0/6)*b^(8.0/3)/10.0/d^(7.0/6);
	yPrev=b*b;
...
\\ pulled out of function above, using the N_a bounds
\\ 10 Dec 2024
get_aLB(b,d,u)={
	my(a2LB,aLB,b4);
	...
	aLB=1;
	\\ from r_0=1, p/q \neq stuff, (AS-3a):
	a2LB=d*b4-sqrt((d/2.2)^23*u^44/b^48);
	if(a2LB>1,
		aLB=floor(sqrt(a2LB));
	);
	\\ from r_0=1, p/q=stuff, (AS-4a):
	a2LB=d*b4-(d/1.5)^13*u^24/b^28;
	if(a2LB>aLB*aLB,
		aLB=floor(sqrt(a2LB));
	);

	\\ from r_0>1, p/q \neq stuff, (AS-5a):
	a2LB=d*b4-sqrt(sqrt((d/38)^11*u^24/b^26));
	if(a2LB>aLB*aLB,
		aLB=floor(sqrt(a2LB));
	);

	\\ from r_0>1, p/q=stuff, (AS-6a):
	a2LB=d*b4-(d*d*u*u*u*u/4225/b4);
	if(a2LB>aLB*aLB,
		aLB=floor(sqrt(a2LB));
	);
	return(aLB);
...
get_dBnd(b,u,dbg=0)={
	...
	dBndTemp=294.0*b^6/u^(24/7);
	if(dBndTemp>dBnd,
		dBnd=max(dBnd,dBndTemp);
		ineqUsed=5;
	);
	dBndTemp=4225.0*b^8/u^4;
	if(dBndTemp>dBnd,
		dBnd=max(dBnd,dBndTemp);
		ineqUsed=6;
	);
...
get_dBnd_r0_1_with_a_and_d(a,b,d,u)={
	...
	dBnd=max(dBnd,2.18*(absNrmA*absNrmA*b^48/u^44)^(1/23)); \\ (AS-4a)
	dBnd=max(dBnd,1.47*(absNrmA*b^28/u^24)^(1/13)); \\ (AS-5a)
	return(dBnd);
}