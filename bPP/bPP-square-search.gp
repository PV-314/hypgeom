		...
		if(!issquare(d),
			...
			if(length(solnM4)>0,
				...
				if(u<uUB,
					...
					if((mul==1 && length(solns)>2) || (mul>1 && length(solns)>1),
						printf("UNIT-M1 (%1d): a=%6d, p=%6d, m=%2d, b=%8d, d=%8d, t=%6d, u=%4d: c0=%2d, c1=%6d, indices=%s, solns=%s\n",length(solns),a,p,m,b,d,t,u,c0,c1,indices,solns);
			...
			if(length(solnP4)>0,
				...
				if(u<uUB,
					...
					solns=out[2];
					if((mul==1 && length(solns)>2) || (mul>1 && length(solns)>1),
						printf("UNIT-P1 (%1d): a=%6d, p=%6d, m=%2d, b=%8d, d=%8d, t=%6d, u=%4d: c0=%2d, c1=%6d, indices=%s, solns=%s\n",length(solns),a,p,m,b,d,t,u,c0,c1,indices,solns);
						output_squares(a,b,c0,c1,yM1,y0,yP1,t,u);
					);
				);
			);
