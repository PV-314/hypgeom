...
\\ and https://www.alpertron.com.ar/QUAD.HTM
...
\\
\\ returns the indices of the squares and the distinct square roots of the squares found
\\ 24 Jan 2020
	...
	if(c0*c1!=0 && denominator(c0)==1 && denominator(c1)==1 && gcd(c0,c1)==1 && polisirreducible(x^2-c1*x+c0) && t*u!=0,
		...
		for(i=1,80,
			...
			if(yNegCurr>0 && denominator(yNegCurr)==1 && issquare(yNegCurr),
				...
				\\	\\print("BIG: added i:=",-i,": yNegCurr:=",yNegCurr,": for d:=",d,": a:=",a,": b:=",b,": t:=",t,": u:=",u,": solns:=",solns,":");
				);
			...
			\\if(a==7 && b==1 && d==5 && t==1,
			\\	print("i=",i,", yT=",yT,", yNegCurr=",yNegCurr,", yNegPrev=",yNegPrev,", yM1=",yM1);
			\\);
			yNegPrev=yNegCurr;
			yNegCurr=yT;
		);
	);
	return([indices,solns]);
}
